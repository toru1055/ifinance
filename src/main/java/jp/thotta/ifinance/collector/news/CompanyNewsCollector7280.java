 ...
 * @author toru1055
 */
public class CompanyNewsCollector7280
        extends BaseCompanyNewsCollector
        implements CompanyNewsCollector {
    private static final int stockId = 7280;
    private static final String IR_URL = "http://www.mitsuba.co.jp/";
    ...
    public void parseIRList(List<CompanyNews> newsList)
        ...
        Elements dtList = doc.select("#home-news > dl > dt");
        Elements ddList = doc.select("#home-news > dl > dd");
        for (int i = 0; i < dtList.size(); i++) {
            Element dt = dtList.get(i);
            Element dd = ddList.get(i);
            String aTxt = dt.select("time").first().text();
